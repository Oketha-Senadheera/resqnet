...
public class DonationItemsCatalogDAO {
    ...
    public Optional<DonationItemsCatalog> findByName(String itemName) {
        String sql = "SELECT item_id, item_name, category FROM donation_items_catalog WHERE item_name = ?";
        try (Connection con = DBConnection.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Error fetching donation item by name", e);
        }
        return Optional.empty();
    }
