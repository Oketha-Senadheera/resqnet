    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (requestIdParam == null || requestIdParam.trim().isEmpty()) {
                resp.sendRedirect(req.getContextPath() + "/gn/donation-requests?error=missing_id");
                return;
            }
            ...
            // Approve the request
            donationRequestDAO.approve(requestId, user.getId());

            // Redirect back with success message
            resp.sendRedirect(req.getContextPath() + "/gn/donation-requests?success=verified");

        } catch (NumberFormatException e) {
            resp.sendRedirect(req.getContextPath() + "/gn/donation-requests?error=invalid_id");
        } catch (Exception e) {
            e.printStackTrace();
