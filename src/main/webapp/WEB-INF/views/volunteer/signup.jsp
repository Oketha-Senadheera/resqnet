...
<layout:auth title="Volunteer Sign Up">
  ...
  <jsp:body>
    <div class="signup-container">
      <h1 class="signup-heading">${not empty generalUser ? 'Upgrade to Volunteer' : 'Volunteer Registration'}</h1>
      
      <c:if test="${not empty error}">
        <div class="error-message">${error}</div>
      </c:if>
      
      <form id="volunteerForm" method="post" action="${not empty generalUser ? pageContext.request.contextPath.concat('/general/be-volunteer') : pageContext.request.contextPath.concat('/signup/volunteer')}" novalidate>
        ...
        <div class="form-actions">
          <a href="${not empty generalUser ? pageContext.request.contextPath.concat('/general/dashboard') : pageContext.request.contextPath.concat('/signup')}" class="btn">Back</a>
          <button type="submit" class="btn btn-primary">${not empty generalUser ? 'Upgrade to Volunteer' : 'Register'}</button>
        </div>
      </form>
    </div>
    <script>
      document.addEventListener('DOMContentLoaded', () => {
        ...
        form.addEventListener('submit', (e) => {
          // Only validate password if the fields exist (new signup, not upgrade)
