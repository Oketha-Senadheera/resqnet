...
<layout:dmc-dashboard pageTitle="ResQnet - Disaster Reports" activePage="disaster-reports">
  <jsp:attribute name="styles">
    <style>
      ...
      /* action pills (reuse pattern from other pages) */
      ...
      @media (max-width:780px){ table.report-table thead { display:none; } table.report-table tbody td { display:block; padding:0.6rem 0.85rem; } table.report-table tbody tr { border-bottom:1px solid var(--color-border); } table.report-table tbody td::before { content:attr(data-label); display:block; font-weight:600; margin-bottom:0.25rem; } .action-pills{ flex-wrap:wrap; } }
    </style>
  </jsp:attribute>
  ...
  <jsp:body>
    ...
    <c:if test="${param.success == 'verified'}">
      <div class="alert alert-success">Report verified successfully!</div>
    </c:if>
    <c:if test="${param.success == 'rejected'}">
      <div class="alert alert-success">Report rejected successfully!</div>
    </c:if>
    <c:if test="${param.error == 'verify'}">
      <div class="alert alert-error">Error verifying report. Please try again.</div>
    </c:if>
    <c:if test="${param.error == 'reject'}">
      <div class="alert alert-error">Error rejecting report. Please try again.</div>
    </c:if>
          ...
          <tbody>
            <c:choose>
              <c:when test="${empty pendingReports}">
                <tr><td colspan="7" style="text-align:center; padding:2rem;">No pending reports</td></tr>
              </c:when>
              <c:otherwise>
                <c:forEach var="report" items="${pendingReports}">
                  <tr>
                    ...
                    <td data-label="Disaster Type" class="type-strong">
                      <c:choose>
                        <c:when test="${report.disasterType == 'Other'}">${report.otherDisasterType}</c:when>
                        <c:otherwise>${report.disasterType}</c:otherwise>
                      </c:choose>
                    </td>
                    ...
                    <td data-label="Actions">
                      <div class="action-pills">
                        ...
                        <form method="post" action="${pageContext.request.contextPath}/dmc/disaster-reports/reject" style="display:inline;">
                          <input type="hidden" name="reportId" value="${report.reportId}" />
                          <button type="submit" class="pill pill-danger" onclick="return confirm('Are you sure you want to reject this report?');">
          ...
          <tbody>
            <c:choose>
              <c:when test="${empty approvedReports}">
                <tr><td colspan="6" style="text-align:center; padding:2rem;">No approved reports</td></tr>
              </c:when>
              <c:otherwise>
